...
from app.models import Track, Snapshot, PlayHistory, DailyPlay, init_database, get_session_context
...
class SQLiteBackend(DatabaseBackend):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.error("sqlite_backend_connection_failed", db_path=str(self.db_path), error=str(e))
            self._connected = False
            return False
    ...
    def upsert_track(self, track_data: Dict):
        ...
        with get_session_context() as session:
            ...
            timestamp = datetime.fromisoformat(track_data.get('last_seen', datetime.now().isoformat()))
            ...
            if track:
                ...
                # Insert new
                first_seen = datetime.fromisoformat(track_data.get('first_seen', timestamp.isoformat()))
                track = Track(
    ...
    def create_snapshot(self, snapshot_data: Dict) -> int:
        ...
        with get_session_context() as session:
            ...
            snapshot = Snapshot(
                ...
                collection_duration_seconds=snapshot_data.get('collection_duration_seconds'),
    ...
    def upsert_daily_play(self, daily_data: Dict):
        ...
        with get_session_context() as session:
            ...
            stmt = (
                select(DailyPlay)
                .where(
                    DailyPlay.persistent_id == persistent_id,
                    DailyPlay.date == date_str
                )
            )
    ...
    def query_top_tracks(self, start_date: str, end_date: str, limit: int) -> List[Dict]:
        ...
        with get_session_context() as session:
            ...
            return [{'name': name, 'artist': artist, 'plays': plays} for name, artist, plays in results]
    ...
    def query_top_albums(self, start_date: str, end_date: str, limit: int) -> List[Dict]:
        ...
        with get_session_context() as session:
            ...
            return [{'album': album, 'artist': artist, 'plays': plays} for album, artist, plays in results]
    ...
    def query_most_skipped(self, start_date: str, end_date: str, limit: int) -> List[Dict]:
        ...
        with get_session_context() as session:
            ...
            return [{'name': name, 'artist': artist, 'skips': skips} for name, artist, skips in results]
        ...
        with get_session_context() as session:
            stmt = (
                select(
                    ...
                    func.count(func.distinct(DailyPlay.persistent_id)).label('unique_tracks'),
                    func.sum(DailyPlay.plays_delta * Track.duration).label('total_time')
                )
                .join(Track)
                .where(
                    DailyPlay.date >= start_date,
    ...
    def query_stats(self) -> Dict:
        ...
        with get_session_context() as session:
            ...
            return {
                ...
                'earliest_snapshot': earliest_snapshot.timestamp.isoformat() if earliest_snapshot else None,
                'latest_snapshot': latest_snapshot.timestamp.isoformat() if latest_snapshot else None
            }
