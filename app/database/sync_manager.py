...
class SyncManager:
    ...
    def __init__(self, sqlite_backend: SQLiteBackend, couchbase_backend: CouchbaseBackend,
        ...
        if os.path.exists(self.state_file):
            try:
                with open(self.state_file, 'r') as f:
                    state = json.load(f)
                    logger.debug(f"Loaded sync state: last_synced_snapshot_id={state.get('last_synced_snapshot_id')}")
                    return state
            except Exception as e:
                logger.warning(f"Failed to load sync state file: {str(e)}, starting fresh")
    ...
    def _save_sync_state(self):
        ...
        try:
            ...
            logger.debug(f"Saved sync state: last_synced_snapshot_id={self.sync_state['last_synced_snapshot_id']}")
        except Exception as e:
    ...
    def needs_sync(self) -> bool:
        ...
        if unsynced:
            logger.info(f"Found {len(unsynced)} unsynced snapshots (IDs: {[s['snapshot_id'] for s in unsynced]})")
            return True
        else:
    ...
    def sync(self) -> bool:
        ...
        unsynced_snapshots = [s for s in all_snapshots if s['snapshot_id'] > last_synced]

        if not unsynced_snapshots:
            logger.info("No snapshots to sync")
            return True

        logger.info(f"Syncing {len(unsynced_snapshots)} snapshots (IDs: {[s['snapshot_id'] for s in unsynced_snapshots]})")
        ...
        for snapshot in sorted(unsynced_snapshots, key=lambda x: x['snapshot_id']):
            ...
            try:
                ...
                self.sync_state['last_sync_status'] = f'failed_at_snapshot_{snapshot_id}'
                self._save_sync_state()
                return False

        logger.info(f"Sync completed successfully, synced {len(unsynced_snapshots)} snapshots")
        return True
    ...
    def _sync_snapshot(self, snapshot: Dict):
        ...
        # 3. Sync play_history for this snapshot
        logger.debug(f"Syncing {len(play_history)} play history records for snapshot {snapshot_id}")
        for history in play_history:
            self._sync_play_history(history)

        # 4. Sync daily_plays (find relevant date range from play_history)
        if play_history:
            dates = {ph.get('played_date') for ph in play_history if ph.get('played_date')}
        ...
        try:
            ...
            if existing:
                ...
                if new_last_seen > existing_last_seen:
                    ...
                    logger.debug(f"Updated track {track['persistent_id']} (newer version from SQLite)")
                else:
                    logger.debug(f"Skipped track {track['persistent_id']} (Couchbase version is newer)")
            else:
    ...
    def _sync_snapshot_metadata(self, snapshot: Dict):
        ...
        try:
            ...
            if exists:
                logger.debug(f"Snapshot {snapshot_id} already exists in Couchbase, skipping")
                return
            ...
            try:
                current_counter = self.couchbase.collections['counters'].get('snapshot_id')
                current_value = current_counter.content_as[int]

                if snapshot_id > current_value:
                    # Set counter to at least this snapshot_id
                    self.couchbase.collections['counters'].upsert('snapshot_id', snapshot_id)
            ...
            logger.error(f"Failed to sync snapshot metadata {snapshot['snapshot_id']}: {str(e)}")
            raise
    ...
    def _sync_play_history(self, history: Dict):
        ...
        try:
            ...
            if exists:
                logger.debug(f"Play history for snapshot {snapshot_id}, track {persistent_id} already exists, skipping")
                return

            self.couchbase.insert_play_history(history)
            logger.debug(f"Synced play history for snapshot {snapshot_id}, track {persistent_id}")
