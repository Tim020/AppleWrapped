...
from app.models import Track, Snapshot, PlayHistory, DailyPlay, init_database, get_session, get_session_context
from app.logging_config import logger


def get_track_count():
    """Get total number of tracks in library"""
    cmd = ['osascript', '-e', 'tell application "Music" to get count of tracks of library playlist 1']
...
def get_track_properties(index):
    ...
    cmd = ['osascript', '-e', f'tell application "Music" to get properties of track {index} of library playlist 1']
    result = subprocess.run(cmd, capture_output=True, text=True)
...
def collect_library_snapshot(max_workers=20):
    ...
    def update_progress():
        with progress_lock:
            ...
            if completed[0] % 100 == 0 or completed[0] == total_tracks:
                ...
                logger.info(f"Collection progress: {completed[0]}/{total_tracks} ({percent}%), ETA {round(eta)}s")
    ...
    logger.info(f"Collection complete: {len(tracks)} tracks collected in {round(collection_duration, 1)}s")
...
def store_snapshot(backend_manager, tracks, collection_duration):
    ...
    prev_play_counts = {}
    if prev_snapshot:
        prev_history = backend_manager.get_play_history_for_snapshot(prev_snapshot['snapshot_id'])
    ...
    logger.info(f"Snapshot storage complete: ID {snapshot_id}, {new_tracks} new tracks, {updated_tracks} updated tracks")
...
def generate_wrapped_report(backend_manager, start_date=None, end_date=None, limit=10):
    ...
    if top_albums:
        ...
        for i, album in enumerate(top_albums, 1):
            print(f"   {i:2d}. {album['album']} - {album['artist']} ({album['plays']} plays)")
    ...
    year = start_date[:4]
    new_tracks_count = sum(1 for t in all_tracks
                          if t.get('date_added') and year in t.get('date_added', ''))
