...
def mock_couchbase_backend():
    ...
    backend.get_play_history_for_snapshot.return_value = []
    backend.get_daily_play.return_value = None
    backend.collections = {"snapshots": Mock(), "counters": Mock()}
    return backend
...
class TestSyncManagerStateManagement:
    ...
    def test_load_nonexistent_state_file(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
        """Test loading when state file doesn't exist"""
        manager = SyncManager(
            mock_sqlite_backend, mock_couchbase_backend, temp_state_file
        )
    ...
    def test_load_existing_state_file(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_load_corrupted_state_file(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
...
class TestSyncManagerNeedsSync:
    ...
    def test_needs_sync_when_new_snapshots_exist(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_needs_sync_when_no_new_snapshots(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_needs_sync_when_no_snapshots(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
...
class TestSyncManagerSync:
    ...
    def test_sync_multiple_snapshots(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_sync_incremental_only_new_snapshots(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_sync_no_snapshots_to_sync(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_sync_failure_updates_state(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
        ...
        mock_couchbase_backend.collections[
            "snapshots"
        ].upsert.side_effect = fail_on_snapshot_2
        ...
        assert (
            manager.sync_state["last_synced_snapshot_id"] == 1
        )  # Only first snapshot succeeded
        assert "failed" in manager.sync_state["last_sync_status"]

    def test_sync_resumes_from_last_successful(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
        ...
        mock_couchbase_backend.collections[
            "snapshots"
        ].upsert.side_effect = fail_on_snapshot_2_first_time
...
class TestSyncManagerTrackSync:
    ...
    def test_sync_track_newer_version(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
    ...
    def test_sync_track_older_version(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
...
class TestSyncManagerDailyPlaySync:
    ...
    def test_sync_daily_play_merge(
        self, mock_sqlite_backend, mock_couchbase_backend, temp_state_file
    ):
        ...
        mock_couchbase_backend.upsert_daily_play.assert_called_once_with(
            expected_merged
        )
